  ...
  <header>
    ...
    <aside class="searchKey">
      ...
      <datalist id="esDataList">
        <option v-for="item in allContext" :id="item.id"
          >{{ item.sysName }}-{{ item.ip }}</option
        >
      </datalist>
    </aside>
  </header>
...
export default {
  ...
  watch: {
    keyword: function(val) {
      /*console.log(val)
        this.allContext.forEach(function (item) {
          if((item.item.sysName + "-" + item.ip).indexOf(val)) {
...
.searchKey input:focus {
  box-shadow: 0px 0px 10px 1px #3f51b5, 0 0 1px #3f51b5, 0 0 1px #3f51b5,
    0 0 1px #3f51b5, 0 0 1px #3f51b5, 0 0 1px #3f51b5, 0 0 1px #3f51b5;
}
</style>
