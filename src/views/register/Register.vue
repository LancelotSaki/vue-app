<template>
  <div class="registerPage">
    <main class="registerMain">
      <ul class="registerContainer">
        <li>
          <input type="radio" id="registerTab1" name="tab" checked><label for="registerTab1">邮箱注册</label>
          <form id="registerCon1" @submit="checkForm1" action="https://www.baidu.com"
                method="post">
            ...
            <div class="alterTab1" :data-words="checkBox.emailCheck" :data-alter="checkBox.emailCodeCheck">{{checkBox.passwordCheck}}</div>
          </form>
        </li>
        <li>
          <input type="radio" name="tab" id="registerTab2"><label for="registerTab2">手机注册</label>
          <form id="registerCon2" @submit="checkForm2" action="https://www.baidu.com"
                method="post">
            <input type="text" placeholder="手机号码" name="phone"
                   maxlength="11" v-model="formValue.phone">
            <input type="password" placeholder="密码" name="password"
                   maxlength="35" v-model="formValue.phonePassword">
            <input type="text" placeholder="验证码" name="phoneCode"
                   maxlength="6" v-model="formValue.phoneCode">
            <input type="button" :value="leftTime === 60 ? '发 送': leftTime + '秒'" :disabled="sendClickDisable"
                   @click="clickSend">
            <input type="submit" value="注 册">
            <div class="alterTab2" :data-words="checkBox.phoneCheck" :data-alter="checkBox.phoneCodeCheck">{{checkBox.password}}</div>
          </form>
        </li>
      </ul>
...
/*隐藏radio*/
.registerContainer input[id^="registerTab"] {
  display: none;
}

/* 将radio相邻的label标签美化一下，可以在这里设置按钮的背景图标*/
.registerContainer input[id^="registerTab"] + label {
  height: 35px;
  width: 110px;
  text-align: left;
  float: right;
...
.registerContainer input[id^="registerTab"] + label:hover {
  cursor: pointer;
}
...
[id^="registerCon"] > input {
  margin-bottom: 30px;
  border: 0;
  padding-left: 7px;
}

[id^="registerCon"] > input:first-child,
[id^="registerCon"] > input:nth-child(2) {
  width: 100%;
  height: 35px;
  border-radius: 5px;
}

[id^="registerCon"] > input:nth-child(3) {
  height: 35px;
  border-radius: 5px;
  width: 65%;
}
...
/*这是一个发送短信按键*/
[id^="registerCon"] > input:nth-child(4) {
  height: 35px;
  border-radius: 5px;
  background: #f0ad24;
  color: white;
...
[id^="registerCon"] > input:nth-child(4):disabled {
  background: #a9a9a9;
}

[id^="registerCon"] > input:nth-child(4):hover {
  background: #99ce5f;
}

[id^="registerCon"] input[type="submit"] {
  height: 35px;
  border-radius: 5px;
  background: #f0ad24;
  color: white;
...
[id^="registerCon"] input[type="submit"]:hover {
  height: 35px;
  border-radius: 5px;
  background: #99ce5f;
  color: white;
